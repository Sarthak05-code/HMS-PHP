...
$test = $conn->query("SELECT 1 FROM attendance LIMIT 1");
if (!$test) die("DB Error: " . $conn->error);
...
<style>
.att-toggle { display:flex; gap:.4rem; }
.att-btn {
  padding:.3rem .75rem; border-radius:6px; font-size:.8rem;
  font-family:inherit; cursor:pointer; border:1px solid var(--border);
  background:var(--input-bg); color:var(--muted); transition:all .15s;
}
.att-btn.present.active { background:rgba(52,211,153,.15); border-color:rgba(52,211,153,.4); color:var(--success); }
.att-btn.absent.active  { background:rgba(248,113,113,.15); border-color:rgba(248,113,113,.4); color:var(--danger); }
.att-btn.leave.active   { background:rgba(251,191,36,.12);  border-color:rgba(251,191,36,.3);  color:var(--warning); }
.pct-bar { display:flex; height:8px; border-radius:4px; overflow:hidden; width:100px; background:rgba(255,255,255,.05); }
.pct-present { background:var(--success); }
.tab-btns { display:flex; gap:.5rem; margin-bottom:1.5rem; }
.tab-btn {
  padding:.5rem 1.1rem; border-radius:8px; font-family:inherit;
  font-size:.88rem; cursor:pointer; border:1px solid var(--border);
  background:transparent; color:var(--muted); transition:all .15s;
}
.tab-btn.active { background:rgba(79,142,247,.15); color:var(--accent); border-color:rgba(79,142,247,.3); }
.bulk-bar {
  display:flex; gap:.75rem; align-items:center;
  background:var(--surface); border:1px solid var(--border);
  border-radius:10px; padding:.75rem 1rem; margin-bottom:1rem; flex-wrap:wrap;
}
.bulk-bar span { font-size:.83rem; color:var(--muted); }
</style>
</head>
<body>
<div class="layout">
      ...
      <form method="POST">
        ...
        <div class="card" style="padding:0;overflow:hidden">
          ...
          <div class="table-wrap">
            <table>
              <thead>
                <tr><th>#</th><th>Name</th><th>Section</th><th>Class</th><th>Attendance</th></tr>
              </thead>
              <tbody>
          ...
          <?php else: ?>
            <div class="empty-state"><div class="empty-icon">📋</div><p>No active students found.</p></div>
          <?php endif; ?>
        </div>
      </form>
    </div>
    ...
    <div id="tab-report" style="display:none">
      ...
      <div class="card" style="padding:0;overflow:hidden">
        ...
        <div class="table-wrap">
          <table>
            <thead>
              <tr><th>#</th><th>Name</th><th>Section</th><th>Present</th><th>Absent</th><th>Leave</th><th>Rate</th></tr>
            </thead>
            <tbody>
            <?php $i=1; while($r=$report->fetch_assoc()):
        ...
        <?php else: ?>
          <div class="empty-state"><div class="empty-icon">📊</div><p>No attendance data for this month.</p></div>
        <?php endif; ?>
      </div>
    </div>
